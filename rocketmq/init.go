...
func main() {
	...
	Message := &Message{
		...
		Subdomain:      "baidu.com",
		Ip:             "baidu.com",
		MonitoringFunc: []string{"1", "2", "3"},
	}
	msg, _ := json.Marshal(Message)
	SendSyncMessage(string(msg))
	// 3.消费者订阅主题并消费
	SubscribeMessage()
}

func CreateTopic(topicName string) {
	endPoint := []string{"10.10.10.43:9876", "10.10.10.43:9877"}
	// 创建主题
...
func SubscribeMessage() {
	...
	err = c.Subscribe("testTopic01", consumer.MessageSelector{}, func(ctx context.Context, msgs ...*primitive.MessageExt) (consumer.ConsumeResult, error) {
		for i := range msgs {
			var p Message
			err := json.Unmarshal(msgs[i].Body, &p)
			if err != nil {
